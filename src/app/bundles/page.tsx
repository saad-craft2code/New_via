    ...
    <div className="min-h-screen">
      ...
      <div className="bg-teal-600 py-6 px-4 lg:px-8">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-3">
          <div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" /><input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder={lang === "ar" ? "ابحث عن باقة..." : "Search bundles..."} className="w-full pl-9 pr-3 py-3 rounded-xl text-sm border-0 focus:outline-none focus:ring-2 focus:ring-teal-300" /></div>
          <select value={difficulty} onChange={(e) => setDifficulty(e.target.value)} className="px-4 py-3 rounded-xl text-sm border-0 focus:outline-none focus:ring-2 focus:ring-teal-300 bg-white"><option value="all">{lang === "ar" ? "كل المستويات" : "All Levels"}</option><option value="Easy">Easy</option><option value="Moderate">Moderate</option><option value="Challenging">Challenging</option></select>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {sorted.map((bundle) => (
            <a key={bundle.id} href={`/bundles/${bundle.id}`} className="group bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-all border border-slate-100">
              <div className="relative h-56 overflow-hidden"><img src={bundle.coverImage} alt={bundle.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" /><div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" /><div className="absolute bottom-3 left-3 right-3 text-white"><div className="flex items-center gap-2 mb-1"><span className="text-xs px-2 py-0.5 bg-teal-500 rounded-md font-medium">{bundle.durationDays} {t(lang, "days")}</span><span className="text-xs px-2 py-0.5 bg-white/20 backdrop-blur rounded-md">{bundle.difficulty}</span></div><h3 className="text-xl font-bold leading-tight">{bundle.title}</h3></div></div>
              <div className="p-5">
