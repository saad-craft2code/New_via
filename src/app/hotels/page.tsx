      ...
      <div className="bg-[#F8FAFC] py-4 mt-0 border-b border-[#E2E8F0]">
        <div className="tf-container flex flex-col md:flex-row gap-3">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder={t(lang, "search_placeholder")} className="tf-input pl-10" />
          </div>
          <select value={country} onChange={(e) => setCountry(e.target.value)} className="px-4 py-2.5 rounded-xl text-sm bg-white border border-[#E2E8F0] text-[#0F172A] focus:outline-none focus:border-[#2563EB]"><option value="all" className="text-black">{lang === "ar" ? "كل الدول" : "All Countries"}</option>{countries.map((c) => <option key={c} value={c} className="text-black">{c}</option>)}</select>
          <select value={city} onChange={(e) => setCity(e.target.value)} className="px-4 py-2.5 rounded-xl text-sm bg-white border border-[#E2E8F0] text-[#0F172A] focus:outline-none focus:border-[#2563EB]"><option value="all" className="text-black">{lang === "ar" ? "كل المدن" : "All Cities"}</option>{cities.map((c) => <option key={c} value={c} className="text-black">{c}</option>)}</select>
